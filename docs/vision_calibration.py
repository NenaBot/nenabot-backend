...
WINDOW_NAME = "NenaBot Vision Calibration"
TRACKBAR_WINDOW = "Calibration Sliders"
...
def _create_trackbars() -> None:
    ...
    cv2.createTrackbar(
        "Marker mm x10",
        TRACKBAR_WINDOW,
        int(DEFAULTS.marker_size_mm * 10),
        2000,
        _nothing,
    )
...
def _read_settings() -> CalibrationSettings:
    ...
    marker_size_mm = cv2.getTrackbarPos("Marker mm x10", TRACKBAR_WINDOW) / 10.0
    marker_size_mm = max(1.0, marker_size_mm)
...
def detect_and_annotate(
    ...
    if ids is not None and len(marker_corners) > 0:
        ...
        pixels_per_mm = side_px / s.marker_size_mm
    ...
    for contour in contours:
        ...
        if pixels_per_mm:
            ...
            label = f"{w:.0f}x{h:.0f} px"

        cv2.putText(
            annotated,
            label,
            (center[0] - 60, center[1] - 12),
    ...
    cv2.putText(
        annotated,
        f"Detections: {detected} | Markers: {marker_count}",
        (12, 24),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.65,
        (255, 255, 255),
    ...
    return annotated, edges, detected, marker_count, pixels_per_mm
...
def _print_settings(s: CalibrationSettings) -> None:
    ...
    print(f"marker_size_mm={s.marker_size_mm:.1f}")
...
def _run_image_mode(image_path: str) -> None:
    ...
    while True:
        ...
        annotated, edges, _, _, _ = detect_and_annotate(frame, settings)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Interactive vision calibration for NenaBot battery detection"
    )
    parser.add_argument(
    ...
    print("- Move sliders in 'Calibration Sliders' to tune detection")
