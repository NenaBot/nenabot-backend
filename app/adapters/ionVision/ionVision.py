"""HTTP client for the IonVision DMS API.

Documentation: https://olfactomics.github.io/IonVision-API-docs/
"""
...
class IVAdapter:
    ...
    def _request(
        ...
        """Make an HTTP request to the IonVision API."""
        try:
    ...
    def replace_scan_comments(self, comments: dict) -> IVResult:
        """Replace the comments on the ongoing or next scan.

        The /currentScan/comments object can first be fetched
        for editing using GET.
        """
    ...
    def get_results(
        ...
        """Search the scan results stored on the device."""
    ...
    def get_latest_dataobject(self) -> IVResult:
        """Get data object of the latest scan result once processed.

        Please note that it can take some time for the device to
        process the scan result data after a scan has finished.
        """
        return self._request("GET", "results/latest")

    def get_latest_gas_detection(self) -> IVResult:
        """Get gas detection results for the latest scan result."""
        return self._request("GET", "results/latest/gasDetection")

    def get_gas_detection_result(self, id: str) -> IVResult:
        """Get gas detection results for a scan result."""
    ...
    def get_parameter_id(self) -> IVResult:
        """Get the ID of the current parameter preset.

        To access other parameter related functionality,
        use the /parameter/* endpoints.
        """
