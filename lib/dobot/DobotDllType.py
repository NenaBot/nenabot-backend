...
def load():
    """Load the native Dobot shared library.

    Note: the upstream SDK examples load the library via a relative path (e.g. "./DobotDll.dll").
    In this repo, the DLL lives next to this Python file, so we resolve the path relative to
    this module to avoid depending on the current working directory.
    """
    ...
    if platform.system() == "Windows":
        ...
        os.environ["DYLD_FRAMEWORK_PATH"] = ":".join(
            new_entries + existing_framework_entries
        )
